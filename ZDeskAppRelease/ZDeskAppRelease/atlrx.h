...
public:
	...
	REParseError Parse(const RECHAR *szRE, BOOL bCaseSensitive=TRUE)
	{
		...
		if (!bCaseSensitive)
		{
			...
			Checked::wmemcpy_s((RECHAR*) szInput, nSize, szRE, nSize);

			CharTraits::Strlwr(const_cast<RECHAR *>(szInput), nSize);
		}
...
/*		if (szInput != szRE)
	...
	BOOL Match(const RECHAR *szIn, CAtlREMatchContext<CharTraits> *pContext, const RECHAR **ppszEnd=NULL)
	{
		...
		if (!m_bCaseSensitive)
		{
			...
			Checked::wmemcpy_s((RECHAR  *) szInput, nSize, szIn, nSize);
			CharTraits::Strlwr(const_cast<RECHAR *>(szInput), nSize);
		}
