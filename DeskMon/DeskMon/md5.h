...
void MD5BinaryString(unsigned char* szString,int nLen,unsigned char* uszFingerPrint);
...
private:
	...
	inline	uint4	rotate_left(uint4 x, uint4 n)			/* value <shift operator> <shift indicator number> ex. 12 << 2 or 12 >> 2 */
	                 { return ((x << n) | (x >> (32-n))); }
